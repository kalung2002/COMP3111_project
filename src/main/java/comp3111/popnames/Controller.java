...
//    	        ((Group) scene.getRoot()).getChildren().add(F_table);    	 
    ...
    /**
	     * Task 2 to be triggered by get result button 
	     */
	    @FXML
		...
		void task2input() {
			String console_text = "";
			String starting_year = task2Startyear.getText();
		...
		void task2Summary() {
			...
			String Gender = task2Gender.getText();
			String oReport = "Summary for K-th popular name:\n";
			...
			String most_freq_name = AnalyzeNames.get_MostFreqName(starting_year, ending_year, Gender, Kth_value);
			int occurrence = 0;
			for (int i = starting_year; i <= ending_year; i++) {
				occurrence += AnalyzeNames.get_name_Occurrence_by_years(i, Gender, Kth_value, most_freq_name);
				System.out.println(occurrence);
			}
			float percent = (float) occurrence / total_occur;
			
			
			oReport += AnalyzeNames.get_MostFreqName(starting_year, ending_year, Gender, Kth_value)+ " has hold the " + Integer.toString(Kth_value) + 
					"-th rank most often for a total of " + AnalyzeNames.get_MostFreqName_freq(starting_year, ending_year, Gender, Kth_value) + 
					" times among names registered for baby " + gender_report + " born in the period from " +
		            Integer.toString(starting_year) + " to " + Integer.toString(ending_year) +".\n";
			oReport += "The total number of occurrences of " + AnalyzeNames.get_MostFreqName(starting_year, ending_year, Gender, Kth_value) + " is " + occurrence 
					+", which represents " + Float.toString(percent * 100.0f) + "% of total " + gender2_report + " births at the "
					+ Integer.toString(Kth_value) + "-th rank in the period from " + Integer.toString(starting_year) 
		...
		void task2BarChart() {
			...
			//String[] name_output = new String[name.length];
			/*
			 * for (int i = 0; i < name.length; i++) { for(int j = 0; j <
			 * name_output.length; j++) { if(name[i].equals(name_output[j])) { bool
